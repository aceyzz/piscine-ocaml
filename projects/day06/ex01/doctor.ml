...
(*   Updated: 2025/12/27 08:46:30 by cedmulle         ###   ########.fr       *)
(*                                                                            *)
(* ************************************************************************** *)
...
class doctor (initial_name : string) (initial_age : int) (initial_sidekick : People.people) =
  object (self)
    ...
    (* ajouts persos pour tester la private method regenerate *)
    method take_damage (damage : int) =
      _hp <- max 0 (_hp - damage);
      Printf.printf "%s takes %d damage! HP: %d\n" _name damage _hp;
      if _hp = 0 then begin
        Printf.printf "%s is regenerating...\n" _name;
        self#regenerate;
        Printf.printf "Regeneration complete! HP: %d\n" _hp
      end
  end
